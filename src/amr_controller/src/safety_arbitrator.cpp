...
public:
    ...
    ultrasonic_stop_(false)
  {
    ...
    RCLCPP_INFO(get_logger(), "Listening to: %s, %s", lidar_topic.c_str(), ultrasonic_topic.c_str());
...
private:
  ...
  {
    bool combined = lidar_stop_ || ultrasonic_stop_;
    std_msgs::msg::Bool msg;
    msg.data = combined;
    safety_stop_pub_->publish(msg);

    RCLCPP_INFO(get_logger(),
      "Safety arbitration → %s  [lidar=%s | ultrasonic=%s]",
      combined        ? "STOP"  : "CLEAR",
