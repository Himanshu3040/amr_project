...
#include "amr_motion/pd_motion_planner.hpp"
...
namespace amr_motion
{
  PDMotionPlanner::PDMotionPlanner() : Node("pd_motion_planner"),
    kp_(2.0), kd_(0.1), step_size_(0.2), max_linear_velocity_(0.5), max_angular_velocity_(0.5),
    prev_angular_error_(0.0), prev_linear_error_(0.0)
  {
    declare_parameter("kp", kp_);
    declare_parameter("kd", kd_);
  ...
  void PDMotionPlanner::controlLoop()
  {
    ...
    if(!transformPlan(robot_pose.header.frame_id)) {
      RCLCPP_ERROR(get_logger(), "Failed to transform global plan to robot frame");
      return;
    }
    ...
    tf2::Transform robot_tf, next_pose_tf, next_pose_in_robot_tf;
    tf2::fromMsg(robot_pose_stamped.pose, robot_tf);
    tf2::fromMsg(next_pose.pose, next_pose_tf);

    next_pose_in_robot_tf = robot_tf.inverse() * next_pose_tf;
    double linear_error = next_pose_in_robot_tf.getOrigin().getX();
    double angular_error = next_pose_in_robot_tf.getOrigin().getY();
    ...
    geometry_msgs::msg::Twist navigation_cmd_vel;
    navigation_cmd_vel.linear.x = std::clamp(kp_ * linear_error + kd_ * linear_error_derivative, -max_linear_velocity_, max_linear_velocity_);
    navigation_cmd_vel.angular.z = std::clamp(kp_ * angular_error + kd_ * angular_error_derivative, -max_angular_velocity_, max_angular_velocity_);
    navigation_cmd_pub_->publish(navigation_cmd_vel);
  ...
  bool PDMotionPlanner::transformPlan(const std::string & frame)
  {
    ...
    try{
      ...
      RCLCPP_ERROR_STREAM(get_logger(), "Couldn't find transform from " << global_plan_.header.frame_id << " to " << frame);
      return false;
    }
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<amr_motion::PDMotionPlanner>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}