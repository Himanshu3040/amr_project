...
#include "amr_planning/a_star_planner.hpp"
...
namespace amr_planning
{
 AStarPlanner::AStarPlanner() : Node("a_star_planner")
  ...
  double AStarPlanner::manhattanDistance(const GraphNode & a, const GraphNode & goal_node){
    return std::abs(a.x - goal_node.x) + std::abs(a.y - goal_node.y);
  }
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<amr_planning::AStarPlanner>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}